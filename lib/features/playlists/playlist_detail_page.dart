...
class _PlaylistDetailPageState extends State<PlaylistDetailPage> {
  ...
  Future<void> _loadTrackDetails() async {
    ...
    try {
      for (final trackId in _playlist.trackIds) {
        if (_trackDetails.containsKey(trackId)) continue;

        // Fetch track details from Spotify
        final response = await EnhancedSpotifyService.getTrackDetails(trackId);
        if (response != null) {
          setState(() {
            _trackDetails[trackId] = response;
          });
        }
      }
    } catch (e) {
  ...
  Future<void> _exportToSpotify() async {
    if (_playlist.source != 'local') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bu playlist zaten Spotify ile senkronize')),
      );
      return;
    }
  ...
  Future<void> _removeTrack(String trackId) async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Şarkı kaldırıldı')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Şarkı kaldırılamadı')),
      );
    }
  }
