...
class _EnhancedUsernameFieldState extends ConsumerState<EnhancedUsernameField> {
  ...
  Widget build(BuildContext context) {
    ...
    return TextFormField(
      controller: widget.controller,
      textInputAction: TextInputAction.next,
      validator: widget.validator ?? AuthValidators.validateUsername,
  ...
  void _checkUsernameAvailability(String username) async {
    ...
    // For now, we'll simulate based on common usernames
    final commonUsernames = ['admin', 'user', 'test', 'demo', 'guest'];
    final isAvailable = !commonUsernames.contains(username.toLowerCase()) && username.length >= 3;
